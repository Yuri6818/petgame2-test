...
export function updateUI() {
  ...
  if (coinEl) coinEl.textContent = (gameState.coins as number)?.toString() || '150'
  if (dustEl) dustEl.textContent = (gameState.dust as number)?.toString() || '25'
  if (levelEl) levelEl.textContent = (gameState.playerLevel as number)?.toString() || '1'
  if (xpEl) xpEl.textContent = (gameState.playerXP as number)?.toString() || '0'
}
